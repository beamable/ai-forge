	...
	public static class TextMeshProHelper
	{
		...
		public static class UIStyleManager
		{
			...
			static UIStyleManager()
			{
				...
				var tmproAssetFolderPath = TextMeshProHelper.PackageRelativePath;

				if (EditorGUIUtility.isProSkin)
				{
					alignLeft = AssetDatabase.LoadAssetAtPath(
					...
					alignGeoCenter =
						AssetDatabase.LoadAssetAtPath(
							...
							typeof(Texture2D)) as Texture2D;
					alignTop = AssetDatabase.LoadAssetAtPath(
				...
				alignContentA = new[]
				{
					new GUIContent(alignLeft, "Left"), new GUIContent(alignCenter, "Center"),
					new GUIContent(alignRight, "Right"), new GUIContent(alignJustified, "Justified"),
					new GUIContent(alignFlush, "Flush"), new GUIContent(alignGeoCenter, "Geometry Center")
				};

				alignContentB = new[]
				{
					new GUIContent(alignTop, "Top"), new GUIContent(alignMiddle, "Middle"),
					new GUIContent(alignBottom, "Bottom"), new GUIContent(alignBaseline, "Baseline"),
					new GUIContent(alignMidline, "Midline"), new GUIContent(alignCapline, "Capline")
				};
			}
		}
	}

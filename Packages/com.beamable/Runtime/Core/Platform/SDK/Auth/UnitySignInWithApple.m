...
-(void)authorizationController:(ASAuthorizationController *)controller didCompleteWithAuthorization:(ASAuthorization *)authorization
{
    if (self.loginCallback)
    {
        if (@available(iOS 13.0, tvOS 13.0, *)) {
            ...
            NSString* displayName = [NSPersonNameComponentsFormatter localizedStringFromPersonNameComponents:name style:NSPersonNameComponentsFormatterStyleDefault options:0];

            data.idToken = [idToken UTF8String];
            data.displayName = [displayName UTF8String];

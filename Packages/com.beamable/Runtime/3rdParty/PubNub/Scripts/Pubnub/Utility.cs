...
namespace PubNubMessaging.Core
{
	...
	internal static class Utility
	{
		...
		internal static long CheckKeyAndParseLong(IDictionary<string, object> dict, string what, string key)
		{
		...
		// At The moment, I am assuming that a particular string will never come in with different parameters;
		...
		public static string EncodeUricomponent(string s, ResponseType type, bool ignoreComma, bool ignorePercent2fEncode)
		{
			...
			if (_CachedEncodedUriComponents.TryGetValue(s, out encodedUri))
			{
				return encodedUri;
			}
			...
			_CachedEncodedUriComponents[s] = encodedUri;
			return encodedUri;
		}
